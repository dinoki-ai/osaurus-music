...
```json
{
  "plugin_id": "dev.example.osaurus-music",
  "name": "Osaurus Music",
  "version": "0.1.0",
  "secrets": [
    {
...
```bash
# Package the plugin
osaurus tools package dev.example.osaurus-music 0.1.0

# Install locally
osaurus tools install ./dev.example.osaurus-music-0.1.0.zip
...
```bash
swift build -c release && osaurus tools package dev.example.osaurus-music 0.1.0 && osaurus tools install ./dev.example.osaurus-music-0.1.0.zip
```
...
codesign --force --options runtime --timestamp \
  ...
  .build/release/libosaurus-music.dylib
```
